...
public class RewriteMe {
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        ...
        return questions.stream().flatMap(q -> q.getAllAnswers().stream()).anyMatch(answer -> answer.equalsIgnoreCase(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        ...
        return (int)questions.stream().flatMap(q -> q.getAllAnswers().stream()).filter(ans -> ans.equalsIgnoreCase(answerCandidate)).count();
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        ...
        return questions.stream().collect(groupingBy(Question::getCategory, Collectors.mapping(Question::getQuestionString, Collectors.toList())));
    ...
    {
     return questions.stream().filter(q->q.getCategory().equals(c)).map(Question::getAllAnswers).flatMap(Collection::stream)
                .reduce("",(ans1,ans2)->{if(ans1.length() < ans2.length()){ans1=ans2;}return ans1;});
    }
